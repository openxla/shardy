...
class BasicScheduleBuildTest(unittest.TestCase):
  ...
  def test_predicate_based_schedule_example(self):
    ...
    self.assertEqual(len(schedule.schedule_rule_builders), 1)
    self.assertEqual(len(schedule.merge_rule_builders), 1)
    ...
    def custom_schedule_builder(fragment_infos, _):
      ...
      merge_rules = [
          pipeline.FragmentMergeRule(
              sources={fwd, bwd},
              target=pipeline.minimal_create_target_info([fwd, bwd]),
          )
          for fwd, bwd in zip(forward, backward)
