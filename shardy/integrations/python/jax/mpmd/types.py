...
class MpmdConfig:
  ...
  def spmd_mesh(self) -> jax.sharding.Mesh:
    """Returns an explicit mesh for SPMD partitioning purposes.

    Deprecated: Use `sharding_mesh` instead.
    """
...
def _check_no_reserved_name(topology: Topology) -> None:
...
def make_config(
    ...
    *,
    ...
    input_mesh_assignment: NameToMeshAssignment | None = None,
    output_mesh_assignment: NameToMeshAssignment | None = None,
  ...
  _check_no_reserved_name(topology)
  _check_no_reserved_mesh_name(topology)
  _validate_partitioning_options(partitioning_options)

  mesh_names_per_mesh_shape: dict[tuple[tuple[str, int], ...], list[str]] = {}
  for mesh_name, mesh in topology.items():
    axes_names_and_size = tuple((k, v) for k, v in mesh.shape.items())
    mesh_names_per_mesh_shape.setdefault(axes_names_and_size, []).append(
        mesh_name
    )
  ...
  _validate_fragment_merge_rules(fragment_merge_rules)
...
def _validate_fragment_merge_rules(
...
def _validate_partitioning_options(
  ...
  if not part_options.PARTIR_MPMD_OPTIONS.issuperset(partitioning_options):
    raise ValueError(
        '`mpmd_options` contains unsupported options '
        f'{partitioning_options.keys() - part_options.PARTIR_MPMD_OPTIONS}'
    )

  for k, v in partitioning_options.items():
    if k in part_options.PARTIR_MPMD_BOOLEAN_OPTIONS:
      if not isinstance(v, bool):
        raise ValueError(f'Option {k} has value {v}, which is not a boolean.')
    elif k == part_options.PARTIR_MPMD_PIPELINE_SCHEDULE_OPTION:
      if v not in part_options.PARTIR_MPMD_PIPELINE_SCHEDULE_VALUES:
        raise ValueError(
