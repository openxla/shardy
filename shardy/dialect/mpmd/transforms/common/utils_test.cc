...
TEST(MergeFragments, BasicMerge) {
  ...
  FragmentOp merged =
      MergeFragments(f0, f1, rewriter, GetFragmentOriginUnion(f0, f1, rewriter),
                     f0.getMeshNameAttr(), f0.getStageIdAttr());
...
TEST(MergeFragments, SharedOperandsAreDeduplicated) {
  ...
  EXPECT_EQ(merged.getNumOperands(), 2);
...
TEST(MergeFragments, PreservesControlOperands) {
  ...
  FragmentOp merged_fragment =
      MergeFragments(f1, f3, rewriter, GetFragmentOriginUnion(f1, f3, rewriter),
                     f1.getMeshNameAttr(), f1.getStageIdAttr());
