...
void TryMergeWithExistingFragment(FragmentOp fragment_op, OpBuilder& builder) {
  ...
  // 2. Find the nearest preceding same-mesh fragment. No fallback to
  //    earlier fragments to avoid introducing false dependencies.
  FragmentOp merge_target = FindLastFragmentOnMesh(fragment_op);
