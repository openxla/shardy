...
void MergeInferredFragmentWithExisting(FragmentOp fragment_op,
                                       ...
                                       Operation* return_op,
                                       OpBuilder& builder) {
  // Find the last same-mesh fragment in the block to merge into. Merging as
  // late as possible keeps tensors alive for less time.
  ...
  // Find the latest op that produces any operand of the inferred fragment.
  // The merge target must be at or after this point so all operands dominate
  // the merged fragment.  Block arguments have no defining op and dominate
  // everything, so they don't constrain positioning.
  ...
  // If merge_target is before the latest operand producer, try to move it
  // after so all operands dominate the merged fragment.
  if (latest_operand_producer && merge_target != latest_operand_producer &&
      merge_target->isBeforeInBlock(latest_operand_producer)) {
