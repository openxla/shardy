...
func.func @has_backward_dep(%arg0: !mesh_1_tensor) -> !mesh_1_tensor attributes {
  ...
  // expected-error@+1 {{Detected backward dependency but expected forward-only pipeline since there are no transpose fragments: fragment "fragment_m2" mesh="m2" produces a value consumed by fragment "fragment_m1" mesh="m1". In a forward-only pipeline, dependencies must go from earlier meshes to later meshes.}}
...
func.func @has_backward_input_transfer(%arg0: !mesh_3_tensor) -> !mesh_2_tensor attributes {
    "topology"=#mpmd.topology<<"m1": <["x"=2]>>, <"m2": <["x"=2]>>, <"m3": <["x"=2]>>>} {
  // expected-error@+1 {{Detected backward transfer but expected forward-only pipeline since there are no transpose fragments}}
  %0 = mpmd.transfer %arg0 : (!mesh_3_tensor) -> !mesh_1_tensor
  %1 = mpmd.fragment_call<mesh="m1", origin=["f1"(0)]> @fragment_m1(%0) : (!mesh_1_tensor) -> !mesh_1_tensor
...
func.func @has_backward_output_transfer(%arg0: !mesh_1_tensor) -> (!mesh_2_tensor, !mesh_1_tensor) attributes {
    "topology"=#mpmd.topology<<"m1": <["x"=2]>>, <"m2": <["x"=2]>>>} {
  %0 = mpmd.fragment_call<mesh="m1", origin=["stage1"(0)]> @fragment_m1(%arg0) : (!mesh_1_tensor) -> !mesh_1_tensor
  %1 = mpmd.transfer %0 : (!mesh_1_tensor) -> !mesh_2_tensor
  %2 = mpmd.fragment_call<mesh="m2", origin=["stage2"(0)]> @fragment_m2(%1) : (!mesh_2_tensor) -> !mesh_2_tensor
  // expected-error@+1 {{Detected backward transfer but expected forward-only pipeline since there are no transpose fragments}}
  %3 = mpmd.transfer %2 : (!mesh_2_tensor) -> !mesh_1_tensor
  func.return %2, %3 : !mesh_2_tensor, !mesh_1_tensor
}
