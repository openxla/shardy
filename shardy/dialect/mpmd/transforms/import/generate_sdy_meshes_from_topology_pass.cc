...
sdy::MeshOp GetOrCreateEmptyMesh(ModuleOp module_op) {
  ...
  sdy::MeshOp empty_mesh = symbol_table.lookup<sdy::MeshOp>("empty_mesh");
  if (empty_mesh) {
    return empty_mesh;
  }
  ...
  return sdy::MeshOp::create(builder, loc, "empty_mesh",
 ...
 private:
  void runOnOperation() final {
    ...
    for (NamedMeshAttr named_mesh_attr : topology.getMeshes()) {
      StringRef mesh_name = named_mesh_attr.getName();
      sdy::MeshAttr mesh_attr = named_mesh_attr.getMesh();
      Location loc = module_op.getLoc();
      sdy::MeshOp::create(builder, loc, mesh_name, mesh_attr);
    }

    sdy::transformShardings(module_op, [&](sdy::TensorShardingAttr sharding) {
      bool all_empty_dim_shardings =
          llvm::all_of(sharding.getDimShardings(),
                       [](sdy::DimensionShardingAttr dim_sharding) {
                         return dim_sharding.getAxes().empty();
                       });
      if (all_empty_dim_shardings) {
        SymbolTable symbol_table(module_op);
        sdy::MeshOp mesh_op =
            symbol_table.lookup<sdy::MeshOp>(sharding.getMeshName());
        if (mesh_op.getMesh().isMaximal()) {
          return sharding;
        }
        auto empty_mesh = GetOrCreateEmptyMesh(module_op);
        return sdy::TensorShardingAttr::get(
            module_op.getContext(), empty_mesh.getName(),
