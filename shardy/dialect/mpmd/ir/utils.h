...
struct FragmentOrigin {
  ...
  friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os,
    ...
    return os << "{computation_name: \"" << origin.computation_name
              << "\", transpose_count: " << origin.transpose_count << "}";
  }
...
struct FragmentInfo {
  ...
  friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os,
    ...
    for (size_t i = 0; i < info.origins.size(); ++i) {
      os << info.origins[i];
      if (i < info.origins.size() - 1) {
        os << ", ";
      }
    }
    os << "]";
    if (info.stage_id.has_value()) {
      os << ", stage_id: " << info.stage_id.value();
    }
...
struct FragmentMergeRule {
  ...
  friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os,
    ...
    os << "sources: [";
    llvm::interleaveComma(rule.sources, os);
