...
LogicalResult outlineInstruction(Operation* op, ArrayRef<Value> liveInValues,
  ...
  size_t numDirectExternalOperands =
      liveInValues.size() - implicitCaptures.size();
  IRMapping mapping;
  size_t implicitArgIdx = numDirectExternalOperands;
...
struct PerInstructionPartitioningPass
 ...
 private:
  bool partitionAndWrapInstruction(Operation* op) {
    ...
    OwningOpRef<ModuleOp> tempModule;
    func::FuncOp outlinedFunc;
    if (failed(outlineInstruction(op, liveInValues, implicitCaptures, callees,
                                  shardingInfo, paddedArgTypes,
