...
class CollectiveInserter {
 ...
 private:
  ...
  // `inAxesPerDim[d]` for each dimension d, or their LCM divided by the latter,
  // if the former is not divisible by the latter. In other words, the capacity
  // represents how much the output is sharded more than the input along a
  // specific dimension (taking into account padding for non divisible case).
  //
  ...
  std::optional<AxesPerDim> getSlicingAxesPerDim() {
    ...
    for (auto [inAxes, outAxes, dimCapacity] :
      ...
      dimCapacity = std::lcm(outShardedSize, inShardedSize) / inShardedSize;
    }
