...
class CollectiveInserter {
 ...
 private:
  ...
  std::optional<AxesToSliceForAllToAll> getAxesToSliceForAllToAll(
    ...
    int64_t currentAxesSize = 1;
    for (AxisRefAttr axis : currentAxesPerDim[srcDim]) {
      currentAxesSize *= axis.getSize(mesh);
    }

    // After slicing, the srcDim will be sharded by the current axes and the
    // added slicing axes. If the dimension size is not divisible by the product
    // of these axis sizes, we cannot slice.
    if (getTensorShape(result)[srcDim] %
            (currentAxesSize * addedSlicingAxesSize) !=
        0) {
      return std::nullopt;
    }
