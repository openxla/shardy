// RUN: %S/run_sdy_interpreter_test.sh %s %t --replica_count=4 --partition_count=1
...
func.func @main() {
  ...
  // Run the 4 replicas in parallel.
  %res:4 = "interpreter.run_parallel"(%s0, %s1, %s2, %s3) {
    programs = [[@combined_all_to_all],
                [@combined_all_to_all],
                [@combined_all_to_all],
                [@combined_all_to_all]]
  ...
  // concatenating y-shards along Dim 3, then x-shards along Dim 1.
  %r0 = "stablehlo.concatenate"(%res#0, %res#1) {dimension = 3 : i64} : (tensor<2x1x2x1xi32>, tensor<2x1x2x1xi32>) -> tensor<2x1x2x2xi32>
  %r1 = "stablehlo.concatenate"(%res#2, %res#3) {dimension = 3 : i64} : (tensor<2x1x2x1xi32>, tensor<2x1x2x1xi32>) -> tensor<2x1x2x2xi32>
