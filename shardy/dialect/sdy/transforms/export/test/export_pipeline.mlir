...
func.func @main(
  ...
  // CHECK:      %[[RESHARD:.*]] = sdy.reshard %{{.*}} <mesh<["a"=4, "b"=4, "c"=4]>, [{}]> : tensor<192xf32>
...
func.func @main(%arg0: tensor<8x2xi32>, %arg1: tensor<4x2xi32>) -> tensor<12x2xi32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2, "c"=2]>, [{"a"}, {}]> : tensor<8x2xi32>
  // CHECK-NEXT: %1:2 = call @foo(%0, %arg1) {sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=2, "b"=2, "c"=2]>, [{"a"}, {}]>, <mesh<["a"=2, "b"=2, "c"=2]>, [{}, {}]>]>}
  // CHECK-NEXT: %2 = sdy.reshard %1#0 <mesh<["a"=2, "b"=2, "c"=2]>, [{}, {"a"}]> : tensor<8x2xi32>
  // CHECK-NEXT: %3 = sdy.reshard %1#1 <mesh<["a"=2, "b"=2, "c"=2]>, [{}, {"a"}]> : tensor<4x2xi32>
  // CHECK-NEXT: %4 = stablehlo.concatenate %2, %3, dim = 0 {sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=2, "b"=2, "c"=2]>, [{}, {"a"}]>]>}
  // CHECK-NEXT: %5 = sdy.reshard %4 <mesh<["a"=2, "b"=2, "c"=2]>, [{}, {}]> : tensor<12x2xi32>
  // CHECK-NEXT: return %5 : tensor<12x2xi32>
...
func.func @main(%arg0: tensor<8xf32>) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2]>, [{"b"}]> : tensor<8xf32>
  // CHECK-NEXT: %1 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2]>, [{"a"}]> : tensor<8xf32>
  // CHECK-NEXT: %2 = call @foo(%1) : (tensor<8xf32>) -> tensor<8xf32>
  // CHECK-NEXT: %3 = call @bar_0(%0) : (tensor<8xf32>) -> tensor<8xf32>
  %0 = call @foo(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  %1 = call @bar_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  return %1 : tensor<8xf32>
}

// CHECK-LABEL: func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<mesh<["a"=2, "b"=2]>, [{"a"}]>}) -> tensor<8xf32> {
func.func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"a"}]>}) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2]>, [{"b"}]> : tensor<8xf32>
  // CHECK-NEXT: %1 = call @bar_0(%0) : (tensor<8xf32>) -> tensor<8xf32>
  %0 = call @bar_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  return %0 : tensor<8xf32>
}

// CHECK-LABEL: func private @bar_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<mesh<["a"=2, "b"=2]>, [{"b"}]>}) -> tensor<8xf32> {
func.func private @bar_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"b"}]>}) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2]>, [{}]> : tensor<8xf32>
...
// CHECK-LABEL: func @main(%arg0: tensor<8xf32>) -> tensor<8xf32> {
func.func @main(%arg0: tensor<8xf32>) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2, "c"=2]>, [{"b"}]> : tensor<8xf32>
  // CHECK-NEXT: %1 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2, "c"=2]>, [{"a"}]> : tensor<8xf32>
  // CHECK-NEXT: %2 = call @foo(%1) : (tensor<8xf32>) -> tensor<8xf32>
  // CHECK-NEXT: %3 = call @bar_0(%0) : (tensor<8xf32>) -> tensor<8xf32>
  %0 = call @foo(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  %1 = call @bar_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
...
// CHECK-LABEL: func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<mesh<["a"=2, "b"=2, "c"=2]>, [{"a"}]>}) -> tensor<8xf32> {
func.func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"a"}]>}) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2, "c"=2]>, [{"c"}]> : tensor<8xf32>
  // CHECK-NEXT: %1 = call @bar(%0) : (tensor<8xf32>) -> tensor<8xf32>
  %0 = call @bar(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  return %0 : tensor<8xf32>
}

// CHECK-LABEL: func private @bar_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<mesh<["a"=2, "b"=2, "c"=2]>, [{"b"}]>}) -> tensor<8xf32> {
func.func private @bar_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"b"}]>}) -> tensor<8xf32> {
  // CHECK-NEXT: %0 = sdy.reshard %arg0 <mesh<["a"=2, "b"=2, "c"=2]>, [{}]> : tensor<8xf32>
  // CHECK-NEXT: return %0 : tensor<8xf32>
  return %arg0 : tensor<8xf32>
}

// CHECK-LABEL: func private @bar(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<mesh<["a"=2, "b"=2, "c"=2]>, [{"c"}]>}) -> tensor<8xf32> {
