...
// CHECK-NEXT: return %[[DOT]] : tensor<8x16xf32>
// TODO(enver): A better solution would be to reshard LHS to [{},{"z"}] instead of fully replicate.
...
// CHECK-NEXT: %[[RESHARD1:.*]] = sdy.reshard %arg0 <@mesh_xyzt, [{}, {"t":(2)2}, {}]> : tensor<64x8x32xf32>
// CHECK-NEXT: %[[RESHARD2:.*]] = sdy.reshard %arg1 <@mesh_xyzt, [{}, {}, {"t":(1)2}]> : tensor<64x32x16xf32>
// CHECK-NEXT: %[[DOT:.*]] = stablehlo.dot_general %[[RESHARD1]], %[[RESHARD2]], batching_dims = [0] x [0], contracting_dims = [2] x [1] {sdy.sharding = #sdy.sharding_per_value<[<@mesh_xyzt, [{}, {"t":(2)2}, {"t":(1)2}]>]>, sdy.sharding_rule = #sdy.op_sharding_rule<([i, j, l], [i, l, k])->([i, j, k]) {i=64, j=8, k=16, l=32}>} : (tensor<64x8x32xf32>, tensor<64x32x16xf32>) -> tensor<64x8x16xf32>
// CHECK-NEXT: return %[[DOT]] : tensor<64x8x16xf32>
// TODO(enver): A better solution would be that batching dimension is resharded to {"y", "x":(1)2} instead of full-replication.
