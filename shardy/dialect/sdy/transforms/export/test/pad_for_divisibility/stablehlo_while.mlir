...
// CHECK-LABEL: func.func @while_with_all_gather(
// CHECK-SAME:    %[[ARG0:.*]]: tensor<7x8xf32>) -> tensor<7x8xf32> {
func.func @while_with_all_gather(%arg0: tensor<7x8xf32>) -> tensor<7x8xf32> {
  // CHECK-NEXT: %[[CST:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK-NEXT: %[[PAD:.*]] = stablehlo.pad %[[ARG0]], %[[CST]], low = [0, 0], high = [1, 0], interior = [0, 0] : (tensor<7x8xf32>, tensor<f32>) -> tensor<8x8xf32>
  // CHECK-NEXT: %[[SLICE:.*]] = sdy.all_slice [{"x"}, {}] %[[PAD]] out_sharding=<@mesh_4_2, [{"x"}, {}]> : tensor<8x8xf32>
  %0 = sdy.all_slice [{"x"}, {}] %arg0 out_sharding=<@mesh_4_2, [{"x"}, {}]> : tensor<7x8xf32>
  ...
  // CHECK-NEXT: } do {
  // CHECK-NEXT:   %[[NEXT_I:.*]] = stablehlo.add %iterArg_2, %[[C1]] : tensor<i32>
  // CHECK-NEXT:   %[[GATHERED:.*]] = sdy.all_gather [{"x"}, {}] %iterArg out_sharding=<@mesh_4_2, [{}, {}]> : tensor<8x8xf32>
  // CHECK-NEXT:   %[[TRIMMED:.*]] = stablehlo.slice %[[GATHERED]] [0:7, 0:8] {sdy.sharding = #sdy.sharding_per_value<[<@mesh_4_2, [{}, {}]>]>} : (tensor<8x8xf32>) -> tensor<7x8xf32>
  // CHECK-NEXT:   %[[CST_LOOP:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK-NEXT:   %[[PAD_LOOP:.*]] = stablehlo.pad %[[TRIMMED]], %[[CST_LOOP]], low = [0, 0], high = [1, 0], interior = [0, 0] {sdy.sharding = #sdy.sharding_per_value<[<@mesh_4_2, [{}, {}]>]>} : (tensor<7x8xf32>, tensor<f32>) -> tensor<8x8xf32>
  // CHECK-NEXT:   %[[SLICED:.*]] = sdy.all_slice [{"x"}, {}] %[[PAD_LOOP]] out_sharding=<@mesh_4_2, [{"x"}, {}]> : tensor<8x8xf32>
  // CHECK-NEXT:   stablehlo.return %[[SLICED]], %[[NEXT_I]] : tensor<8x8xf32>, tensor<i32>
  // CHECK-NEXT: }
  %1:2 = stablehlo.while(%iterArg = %0, %iterArg_1 = %c0) : tensor<7x8xf32>, tensor<i32>
    attributes {sdy.sharding = #sdy.sharding_per_value<[<@mesh_4_2, [{"x"}, {}]>, <@mesh_4_2, []>]>}
    cond {
      ...
      %step = stablehlo.add %iterArg_1, %c1 : tensor<i32>
      %gathered = sdy.all_gather [{"x"}, {}] %iterArg out_sharding=<@mesh_4_2, [{}, {}]> : tensor<7x8xf32>
      %sliced = sdy.all_slice [{"x"}, {}] %gathered out_sharding=<@mesh_4_2, [{"x"}, {}]> : tensor<7x8xf32>
      stablehlo.return %sliced, %step : tensor<7x8xf32>, tensor<i32>
    }
  // CHECK-NEXT: %[[OUT_GATHER:.*]] = sdy.all_gather [{"x"}, {}] %[[WHILE]]#0 out_sharding=<@mesh_4_2, [{}, {}]> : tensor<8x8xf32>
  // CHECK-NEXT: %[[OUT_TRIM:.*]] = stablehlo.slice %[[OUT_GATHER]] [0:7, 0:8] {sdy.sharding = #sdy.sharding_per_value<[<@mesh_4_2, [{}, {}]>]>} : (tensor<8x8xf32>) -> tensor<7x8xf32>
  %2 = sdy.all_gather [{"x"}, {}] %1#0 out_sharding=<@mesh_4_2, [{}, {}]> : tensor<7x8xf32>
  // CHECK-NEXT: return %[[OUT_TRIM]] : tensor<7x8xf32>
  return %2 : tensor<7x8xf32>
}
