...
// CHECK-LABEL: func @padded_conv_divisible_sharded_output
func.func @padded_conv_divisible_sharded_output(%arg0: tensor<1x4x8x3xf32>, %arg1: tensor<3x3x3x2xf32>) -> tensor<1x2x6x2xf32> {
  // Pad LHS input feature (dim 3) with zero.
  // CHECK: %[[CST0:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK: %[[PAD0:.*]] = stablehlo.pad %arg0, %[[CST0]], low = [0, 0, 0, 0], high = [0, 0, 0, 1], interior = [0, 0, 0, 0] : (tensor<1x4x8x3xf32>, tensor<f32>) -> tensor<1x4x8x4xf32>
  // CHECK: %[[SLICE0:.*]] = sdy.all_slice [{}, {}, {"x"}, {"y"}] %[[PAD0]] out_sharding=<@mesh_2_2, [{}, {}, {"x"}, {"y"}]> : tensor<1x4x8x4xf32>

  // Pad RHS input feature (dim 2) with zero.
  // CHECK: %[[CST1:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK: %[[PAD1:.*]] = stablehlo.pad %arg1, %[[CST1]], low = [0, 0, 0, 0], high = [0, 0, 1, 0], interior = [0, 0, 0, 0] : (tensor<3x3x3x2xf32>, tensor<f32>) -> tensor<3x3x4x2xf32>
  // CHECK: %[[SLICE1:.*]] = sdy.all_slice [{}, {}, {"y"}, {}] %[[PAD1]] out_sharding=<@mesh_2_2, [{}, {}, {"y"}, {}]> : tensor<3x3x4x2xf32>

  // Perform convolution (result shape 6 is divisible by x=2).
  // CHECK: %[[CONV:.*]] = stablehlo.convolution(%[[SLICE0]], %[[SLICE1]])
  // CHECK-SAME: {batch_group_count = 1 : i64, feature_group_count = 1 : i64, sdy.sharding = #sdy.sharding_per_value<[<@mesh_2_2, [{}, {}, {"x"}, {}]>]>} : (tensor<1x4x8x4xf32>, tensor<3x3x4x2xf32>) -> tensor<1x2x6x2xf32>
  // CHECK-NOT: stablehlo.slice
  // CHECK: return %[[CONV]]

  %sliced_lhs = sdy.all_slice [{}, {}, {"x"}, {"y"}] %arg0 out_sharding=<@mesh_2_2, [{}, {}, {"x"}, {"y"}]> : tensor<1x4x8x3xf32>
  %sliced_rhs = sdy.all_slice [{}, {}, {"y"}, {}] %arg1 out_sharding=<@mesh_2_2, [{}, {}, {"y"}, {}]> : tensor<3x3x3x2xf32>
  %conv_out = stablehlo.convolution(%sliced_lhs, %sliced_rhs)
    dim_numbers = [b, 0, 1, f]x[0, 1, i, o]->[b, 0, 1, f],
    window = {
      stride = [1, 1],
