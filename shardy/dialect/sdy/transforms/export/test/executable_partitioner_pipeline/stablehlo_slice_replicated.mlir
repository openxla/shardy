...
func.func @parallel_slice_replicated(
  ...
  %1 = sdy.all_gather [{"b"}] %0 out_sharding=<@mesh, [{}]> : tensor<3xi32>
...
func.func @main() {
  ...
  %res:4 = "interpreter.run_parallel"(%s0, %s1, %s0, %s1) {
    programs = [[@parallel_slice_replicated, @parallel_slice_replicated, @parallel_slice_replicated, @parallel_slice_replicated]]
  } : (tensor<4xi32>, tensor<4xi32>, tensor<4xi32>, tensor<4xi32>) -> (tensor<3xi32>, tensor<3xi32>, tensor<3xi32>, tensor<3xi32>)

  "check.expect_eq"(%res#0, %seq) : (tensor<3xi32>, tensor<3xi32>) -> ()
  "check.expect_eq"(%res#1, %seq) : (tensor<3xi32>, tensor<3xi32>) -> ()
  "check.expect_eq"(%res#2, %seq) : (tensor<3xi32>, tensor<3xi32>) -> ()
  "check.expect_eq"(%res#3, %seq) : (tensor<3xi32>, tensor<3xi32>) -> ()
