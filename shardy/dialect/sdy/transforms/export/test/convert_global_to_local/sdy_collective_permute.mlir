...
// RUN: sdy_opt %s -sdy-convert-global-to-local="replica-count=8 partition-count=1" | FileCheck %s --check-prefixes=CHECK,REPLICA
...
func.func @mesh_change_iota_to_other(
  ...
  // CHECK: %[[RES:.*]] = "stablehlo.collective_permute"(%[[ARG0]]) <{
  // PARTITION-SAME:   channel_handle = #stablehlo.channel_handle<handle = 5, type = 1>,
  // REPLICA-NOT:      channel_handle
...
func.func @mesh_change_other_to_iota(
  ...
  // CHECK: %[[RES:.*]] = "stablehlo.collective_permute"(%[[ARG0]]) <{
  // PARTITION-SAME:   channel_handle = #stablehlo.channel_handle<handle = 6, type = 1>,
  // REPLICA-NOT:      channel_handle
