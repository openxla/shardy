...
// RUN: sdy_opt %s -sdy-convert-global-to-local | FileCheck %s --check-prefixes=CHECK,V3
...
func.func @shard_result(%arg0: tensor<16x32xf32> {sdy.sharding = #sdy.sharding<@mesh_2_4, [{}, {"y":(2)2}]>})
  ...
  // V3-SAME{LITERAL}: replica_groups = dense<[[0, 2], [1, 3], [4, 6], [5, 7]]>
...
func.func @not_shard_result(%arg0: tensor<16x32xf32>)
  ...
  // V3-SAME{LITERAL}: replica_groups = dense<[[0, 2, 4, 6], [1, 3, 5, 7]]>
  // CHECK-SAME: use_global_device_ids
  // CHECK: ^bb0(%[[ACC:.*]]: tensor<f32>, %[[UPD:.*]]: tensor<f32>):
  // CHECK: %[[ADD:.*]] = stablehlo.add %[[ACC]], %[[UPD]] : tensor<f32>
  // CHECK: stablehlo.return %[[ADD]] : tensor<f32>
...
// CHECK-SAME: (%[[ARG0:.*]]: tensor<16x32xf32>) -> tensor<16x32xf32> {
func.func @all_reduce_max(%arg0: tensor<16x32xf32>)
  -> tensor<16x32xf32> {
  // CHECK: %[[RES:.*]] = "stablehlo.all_reduce"(%[[ARG0]])
  // CHECK-SAME: use_global_device_ids
  // CHECK: ^bb0(%[[ACC:.*]]: tensor<f32>, %[[UPD:.*]]: tensor<f32>):
  // CHECK: %[[MAX:.*]] = stablehlo.maximum %[[ACC]], %[[UPD]] : tensor<f32>
  // CHECK: stablehlo.return %[[MAX]] : tensor<f32>
  // CHECK: }) : (tensor<16x32xf32>) -> tensor<16x32xf32>
  %0 = sdy.all_reduce max {"x"} %arg0 out_sharding=<@mesh_2_4, [{}, {}]> : tensor<16x32xf32>
  // CHECK: return %[[RES]] : tensor<16x32xf32>
  return %0 : tensor<16x32xf32>
}
...
// CHECK-SAME: (%[[ARG0:.*]]: tensor<16x32xf32>) -> tensor<16x32xf32> {
func.func @all_reduce_min(%arg0: tensor<16x32xf32>)
  -> tensor<16x32xf32> {
  // CHECK: %[[RES:.*]] = "stablehlo.all_reduce"(%[[ARG0]])
  // CHECK-SAME: use_global_device_ids
  // CHECK: ^bb0(%[[ACC:.*]]: tensor<f32>, %[[UPD:.*]]: tensor<f32>):
  // CHECK: %[[MIN:.*]] = stablehlo.minimum %[[ACC]], %[[UPD]] : tensor<f32>
  // CHECK: stablehlo.return %[[MIN]] : tensor<f32>
  // CHECK: }) : (tensor<16x32xf32>) -> tensor<16x32xf32>
  %0 = sdy.all_reduce min {"x"} %arg0 out_sharding=<@mesh_2_4, [{}, {}]> : tensor<16x32xf32>
  // CHECK: return %[[RES]] : tensor<16x32xf32>
  return %0 : tensor<16x32xf32>
}
