...
func.func @input_dim_split_and_swapped(%arg0: tensor<8x8x8xf32> {sdy.sharding = #sdy.sharding<@mesh2d, [{"x", "y"}, {}, {}]>}) -> tensor<8x8x8xf32> {
  // CHECK-NEXT: %[[CP:.*]] = sdy.collective_permute %arg0 out_sharding=<@mesh2d, [{"y", "x"}, {}, {}]> : tensor<8x8x8xf32>
  // CHECK-NEXT: %[[A2A:.*]] = sdy.all_to_all [{"x"}: 0->1] %[[CP]] out_sharding=<@mesh2d, [{"y"}, {"x"}, {}]> : tensor<8x8x8xf32>
...
func.func @input_dim_split_into_three_groups(%arg0: tensor<16x8x8xf32> {sdy.sharding = #sdy.sharding<@mesh3d, [{"x", "y", "z"}, {}, {}]>}) -> tensor<16x8x8xf32> {
  // CHECK-NEXT: %[[CP:.*]] = sdy.collective_permute %arg0 out_sharding=<@mesh3d, [{"y", "x", "z"}, {}, {}]> : tensor<16x8x8xf32>
  // CHECK-NEXT: %[[A2A_0:.*]] = sdy.all_to_all [{"z"}: 0->2] %[[CP]] out_sharding=<@mesh3d, [{"y", "x"}, {}, {"z"}]> : tensor<16x8x8xf32>
  // CHECK-NEXT: %[[A2A_1:.*]] = sdy.all_to_all [{"x"}: 0->1] %[[A2A_0]] out_sharding=<@mesh3d, [{"y"}, {"x"}, {"z"}]> : tensor<16x8x8xf32>
...
func.func @input_dim_split_with_subaxes(%arg0: tensor<16x8xf32> {sdy.sharding = #sdy.sharding<@mesh2d_2x8, [{"y"}, {}]>}) -> tensor<16x8xf32> {
  // CHECK-NEXT: %[[CP:.*]] = sdy.collective_permute %arg0 out_sharding=<@mesh2d_2x8, [{"y":(2)4, "y":(1)2}, {}]> : tensor<16x8xf32>
  // CHECK-NEXT: %[[A2A:.*]] = sdy.all_to_all [{"y":(1)2}: 0->1] %[[CP]] out_sharding=<@mesh2d_2x8, [{"y":(2)4}, {"y":(1)2}]> : tensor<16x8xf32>
  // CHECK-NEXT: return %[[A2A]]
