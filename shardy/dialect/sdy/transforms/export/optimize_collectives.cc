...
std::optional<SplitConfig> analyzeSplit(const A2AChain& chain,
                                        ...
                                        MLIRContext* ctx) {
  auto tensorType = dyn_cast<RankedTensorType>(chain.input.getType());
  if (!tensorType) {
    return std::nullopt;
  }
  ...
  // Verify that all axes permuted by CP are communicated by the A2A chain.
  TensorShardingAttr cpOutSharding =
      getOrCreateSharding(chain.cp->getResult(0), outSharding.getMeshName());
  if (cpOutSharding.getRank() != inSharding.getRank()) {
    return std::nullopt;
  }
  for (int64_t i = 0; i < inSharding.getRank(); ++i) {
    if (i != *splitDim &&
        cpOutSharding.getDimSharding(i) != inSharding.getDimSharding(i)) {
      return std::nullopt;
    }
  }
  ArrayRef<AxisRefAttr> cpAxes =
      cpOutSharding.getDimSharding(*splitDim).getAxes();
  if (cpAxes.size() != axesToSplit.size()) {
    return std::nullopt;
  }
  SmallVector<AxisRefAttr> permutedAxes;
  for (size_t i = 0; i < axesToSplit.size(); ++i) {
    if (axesToSplit[i] != cpAxes[i]) {
      permutedAxes.push_back(axesToSplit[i]);
    }
  }
  if (!permutedAxes.empty()) {
    SmallVector<AxisRefAttr> communicatedAxes;
    for (AllToAllOp a2a : chain.a2as) {
      communicatedAxes.push_back(a2a.getParams()[0].getAxes()[0]);
    }
    for (AxisRefAttr permutedAxis : permutedAxes) {
      if (llvm::find(communicatedAxes, permutedAxis) ==
          communicatedAxes.end()) {
        return std::nullopt;
      }
    }
  }
  ...
  SmallVector<int64_t> permutedGroup =
      llvm::to_vector(llvm::seq<int64_t>(*splitDim, *splitDim + k));
...
void rewriteA2AChain(AllToAllOp terminalOp, const A2AChain& chain,
  ...
  auto tensorType = cast<RankedTensorType>(chain.input.getType());
  ...
  rewriter.replaceOp(terminalOp, reshape2);
  for (AllToAllOp a2a : llvm::drop_end(chain.a2as)) {
