...
struct RemoveAllGatherReduceScatterForCMV1Pass
  ...
  void runOnOperation() final {
    SmallVector<Operation*> collectivesToErase;
    getOperation()->walk([&](Operation* op) {
      llvm::TypeSwitch<Operation*>(op)
          .Case<sdy::AllGatherOp>([&](sdy::AllGatherOp allGatherOp) {
            for (Operation* user : allGatherOp->getUsers()) {
              if (isa<stablehlo::DotOp, stablehlo::DotGeneralOp>(user)) {
                collectivesToErase.push_back(allGatherOp);
                return;
              }
            }
          })
          .Case<sdy::ReduceScatterOp>([&](sdy::ReduceScatterOp
                                              reduceScatterOp) {
            Operation* operandOp =
                reduceScatterOp->getOperand(0).getDefiningOp();
            if (isa_and_nonnull<stablehlo::DotOp, stablehlo::DotGeneralOp>(
                    operandOp) &&
                reduceScatterOp.getOutSharding().getUnreducedAxes().empty()) {
              collectivesToErase.push_back(reduceScatterOp);
            }
          });
    });

    for (Operation* op : collectivesToErase) {
