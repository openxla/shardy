  ...
  LogicalResult matchAndRewrite(
    ...
    if (!reductionAxes.empty() &&
      ...
      if (op.getNumResults() == 1) {
        ...
        // Instead, append a size-1 dimension to each partial result, all-gather
        // across reductionAxes along that dimension, and apply a second local
        // reduce across the gathered dimension.
        int64_t numShards = 1;
        for (AxisRefAttr axis : reductionAxes) {
          numShards *= axis.getSize(mesh);
        }
        Attribute replicaGroups = getReplicaGroups(
