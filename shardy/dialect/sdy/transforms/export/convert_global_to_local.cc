...
ValueRange createAllReduceForUnreducedAxes(
    Location loc, ValueRange localResults, ArrayRef<AxisRefAttr> reductionAxes,
    MeshAttr mesh, Attribute meshOrRef, bool enableRGV3,
    ConversionState& conversionState, ConversionPatternRewriter& rewriter) {
  Attribute replicaGroups =
      getReplicaGroups(reductionAxes, mesh, meshOrRef, enableRGV3, rewriter);
  auto channelHandle = stablehlo::ChannelHandleAttr::get(
      rewriter.getContext(), conversionState.getNextChannelId(),
  ...
  buildReduceComputation(allReduce, elementType, ReductionOp::SUM, rewriter);
  return allReduce.getResults();
}
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!reductionAxes.empty() && !hasUnreducedAxes) {
      ...
      results = createAllReduceForUnreducedAxes(
          op.getLoc(), results, reductionAxes, mesh, meshOrRef, enableRGV3,
          conversionState, rewriter);
    }
    rewriter.replaceOp(op, results);
      ...
      if (!contractingAxes.empty()) {
        ...
        if (!hasUnreducedResultSharding(op, contractingAxes, "dot",
          ...
          ValueRange results = createAllReduceForUnreducedAxes(
              op.getLoc(), localDot->getResults(), contractingAxes, mesh,
              lhsSharding.getMeshOrRef(), enableRGV3, conversionState,
              rewriter);
          rewriter.replaceOp(op, results);
          conversionState.removeToConvertOp(op);
          return success();
    ...
    if (!isFullyReplicated(lhsSharding)) {
      ...
      if (!reductionAxes.empty()) {
        ...
        if (!hasUnreducedResultSharding(op, reductionAxes, "dot_general",
          ...
          ValueRange results = createAllReduceForUnreducedAxes(
              op.getLoc(), localDotGeneral->getResults(), reductionAxes, mesh,
              lhsSharding.getMeshOrRef(), enableRGV3, conversionState,
              rewriter);
          rewriter.replaceOp(op, results);
          conversionState.removeToConvertOp(op);
...
class StablehloReduceOpPattern
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // Inspect the first input as sharding rule guarantees that all inputs have
    // the same sharding.
    TensorShardingAttr inputSharding =
        converter->getSharding(op.getInputs().front());
...
class StablehloGatherOpPattern
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!hasUnreducedAxes) {
      result =
          createAllReduceForUnreducedAxes(loc, result, reductionAxes, mesh,
                                          operandSharding.getMeshOrRef(),
