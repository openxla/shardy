...
class ReduceScatterOpPattern : public OpConversionPattern<ReduceScatterOp> {
 public:
  ...
  LogicalResult rewriteReduceScatterMultipleDims(
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto [dim, axesList] : llvm::enumerate(op.getReduceScatterAxes())) {
      slicingAxesPerDim.push_back(axesList.getValue());
      if (!axesList.empty()) {
        lastSlicingDim = dim;
        numSlicingDims++;
        for (AxisRefAttr axis : axesList.getValue()) {
          allReduceAxes.push_back(axis);
        }
      }
    }
    ...
    return rewriteReduceScatterMultipleDims(op, adaptor.getTensor(), mesh,
                                            slicingAxesPerDim, allReduceAxes,
                                            rewriter);
  }
 ...
 protected:
  void runOnOperation() final {
    ...
    patterns.add<AllSliceOpPattern, AllToAllOpPattern,
                 CollectivePermuteOpPattern, FuncOpSignaturePattern,
                 GenericOpPattern, ReduceScatterOpPattern, ReturnOpPattern>(
