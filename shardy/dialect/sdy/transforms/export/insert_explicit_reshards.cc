...
SmallVector<AxisListRef> findCommonAxes(const ShardingProjection& projection,
  ...
  if (shardingRule.getNeedReplicationFactors().empty()) {
    if (auto tensorIndexToIgnore = findTensorIndexShardingIsCompatibleWithout(
            projection, shardingRule.getNumFactors());
        tensorIndexToIgnore) {
      SmallVector<AxisListRef> factorAxisRefs = findCommonAxesIgnoringOneTensor(
          *tensorIndexToIgnore, projection, shardingRule.getNumFactors());
