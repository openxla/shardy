  ...
  for (auto [i, dims] :
    ...
    if (llvm::is_contained(lhsFactors, factorIdx) &&
      ...
      if (!isWindowPassthroughDim(
              convOp.getPadding(), numWindows,
              convOp.getWindowStrides().value_or(ArrayRef<int64_t>()), i,
              convOp.getLhsDilation().value_or(ArrayRef<int64_t>()),
              convOp.getRhsDilation().value_or(ArrayRef<int64_t>()))) {
        // If the spatial dim is not a passthrough in the result, it has
        // dual kPermutation semantics. The spatial dim is a passthrough if
        // numWindows, windowStrides, lhsDilation, and rhsDilation are all 1,
        // and the padding is 0.
        return true;
      }
    }
  }
...
void resolvePermutationFactorsViaReplication(Operation* op,
  ...
  auto isRemovedAxis = [&](AxisRefAttr axis) {
    return llvm::any_of(removedReductionAxes, [&](AxisRefAttr removed) {
      return removed == axis || removed.overlaps(axis);
    });
  };

  SmallVector<AxisRefAttr> newUnreduced;
  for (AxisRefAttr axis : oldUnreduced) {
    if (!isRemovedAxis(axis)) {
      newUnreduced.push_back(axis);
    }
  }
  if (newUnreduced.size() == oldUnreduced.size()) {
    return;
  }

  // Remove the dual-semantics reduction axes from the op's result unreduced
  // sharding annotation and update/remove its downstream sdy.all_reduce user.
  sdy::setShardings(op, oldOutSharding.replaceUnreducedAxes(newUnreduced));
  SDY_CHECK(op->getResult(0).hasOneUse());
  auto allReduceOp =
      dyn_cast<sdy::AllReduceOp>(*op->getResult(0).getUsers().begin());
  SDY_CHECK(allReduceOp);
  SDY_CHECK(allReduceOp.getReductionAxes() == oldUnreduced);
