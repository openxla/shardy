...
# Wrapper to run sdy execution tests for the explicit gather/scatter batching
# import pass.
...
"$SPLIT_FILE" "$SRC" "$TMP"
"$SDY_OPT" "$TMP/part1.mlir" --sdy-explicit-gather-scatter-batching \
  --allow-unregistered-dialect > "$TMP/part1_processed.mlir"
sed '1d; /^}/,$d' "$TMP/part1_processed.mlir" > "$TMP/combined.mlir"

# Generate original (untransformed) version as reference baseline.
sed 's/transformed_/original_/g' "$TMP/part1.mlir" >> "$TMP/combined.mlir"
