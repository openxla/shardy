...
bool isUsedByShardingConstraint(Value input, Operation* excluded = nullptr) {
  return llvm::any_of(
      input.getUsers(), [excluded](Operation* user) {
        return user != excluded && isa<ShardingConstraintOp, ManualComputationOp>(user);
      });
}
...
ShardingConstraintOp getTailOfShardingConstraintChain(
  ...
  while (tail->hasOneUse() &&
         isa<ShardingConstraintOp>(tail->getUses().begin()->getOwner())) {
    tail = cast<ShardingConstraintOp>(tail->getUses().begin()->getOwner());
  }
      ...
      TypeSwitch<Operation*>(op)
          .Case<ShardingConstraintOp>(
              [](ShardingConstraintOp shardingConstraintOp) {
                ...
                //
                // Refer to `getFirstShardingConstraintInChainIfShouldReplace`
                // for more details.
                ShardingConstraintOp head = shardingConstraintOp;
                if (ShardingConstraintOp tail =
                        getTailOfShardingConstraintChain(
