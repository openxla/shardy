...
void addPropagationPipeline(OpPassManager& pm,
  ...
  PropagationOptions optionWithKeepShardingRules = options;
  optionWithKeepShardingRules.keepShardingRules = true;
  addImportPipeline(pm, options.dumpDirectory, options.skipInline);
  pm.addPass(createUserPriorityPropagationPass(optionWithKeepShardingRules));
