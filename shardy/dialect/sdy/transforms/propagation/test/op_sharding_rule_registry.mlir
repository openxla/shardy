...
func.func @reshape_swap_dims_no_common_divisor(%arg0: tensor<3x2xf32>) -> tensor<2x3xf32> {
  // CHECK: #sdy.op_sharding_rule<([i, k])->([i, j]) {i=3, j=3, k=2} need_replication={j, k} permutation={i}>
...
func.func @reshape_swap_dims_with_common_divisor(%arg0: tensor<6x4xf32>) -> tensor<4x6xf32> {
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([ij, lm])->([ij, km]) {i=2, j=3, k=3, l=2, m=2} need_replication={k, l} permutation={j}>
...
func.func @reshape_swap_with_dim_before_and_in_between(%arg0: tensor<5x2x5x3xf32>) -> tensor<5x3x5x2xf32> {
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([i, j, k, m])->([i, j, l, n]) {i=5, j=2, k=5, l=5, m=3, n=2} need_replication={k, l, m, n} permutation={j}>
...
func.func @reshape_swap_and_merge(%arg0: tensor<2x3x8xf32>) -> tensor<3x4x4xf32> {
  // CHECK: dy.sharding_rule = #sdy.op_sharding_rule<([i, j, lm])->([i, kl, m]) {i=2, j=3, k=2, l=2, m=4} need_replication={j, k} permutation={i}>
...
func.func @reshape_split_and_swap(%arg0: tensor<3x4x4xf32>) -> tensor<2x3x8xf32> {
  // CHECK: dy.sharding_rule = #sdy.op_sharding_rule<([i, kl, m])->([i, j, lm]) {i=3, j=3, k=2, l=2, m=4} need_replication={j, k} permutation={i}>
...
func.func @reshape_split_swap_and_merge(%arg0: tensor<4x7x5x8xf32>) -> tensor<14x10x8xf32> {
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([ij, k, m, n])->([ij, lm, n]) {i=2, j=2, k=7, l=2, m=5, n=8} need_replication={k, l} permutation={j}>
...
func.func @scatter_single_input(%arg0: tensor<3x4x2xf32>, %arg1: tensor<2x3x2xi64>, %arg2: tensor<2x3x2x2xf32>) -> tensor<3x4x2xf32>{
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([n, k, m], [i, j, o], [i, j, l, m])->([n, k, m]) {i=2, j=3, k=4, l=2, m=2, n=3, o=2} reduction={i, j} need_replication={k, l, o}>
...
func.func @scatter_single_input_with_implicit_index_vector_dim(%arg0: tensor<3x4x2xf32>, %arg1: tensor<2x3xi64>, %arg2: tensor<2x3x2x2xf32>) -> tensor<3x4x2xf32>{
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([n, k, m], [i, j], [i, j, l, m])->([n, k, m]) {i=2, j=3, k=4, l=2, m=2, n=3} reduction={i, j} need_replication={k, l}>
  %0 = "stablehlo.scatter"(%arg0, %arg1, %arg2) ({
    ^bb0(%arg3: tensor<f32>, %arg4: tensor<f32>):
      %1 = stablehlo.add %arg3, %arg4 : tensor<f32>
      stablehlo.return %1 : tensor<f32>
...
func.func @scatter_inserted_window_dim_is_last_one(%arg0: tensor<4x2x3xf32>, %arg1: tensor<2x3x2xi64>, %arg2: tensor<2x3x2x2xf32>) -> tensor<4x2x3xf32>{
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([k, m, n], [i, j, o], [i, j, l, m])->([k, m, n]) {i=2, j=3, k=4, l=2, m=2, n=3, o=2} reduction={i, j} need_replication={k, l, o}>
  %0 = "stablehlo.scatter"(%arg0, %arg1, %arg2) ({
    ^bb0(%arg3: tensor<f32>, %arg4: tensor<f32>):
      %1 = stablehlo.add %arg3, %arg4 : tensor<f32>
      stablehlo.return %1 : tensor<f32>
...
func.func @scatter_batching_dims(%arg0: tensor<5x3x7x4xf32>, %arg1: tensor<7x5x3x2xi64>, %arg2: tensor<7x5x3x2xf32>) -> tensor<5x3x7x4xf32> {
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([j, n, i, l], [i, j, k, o], [i, j, k, m])->([j, n, i, l]) {i=7, j=5, k=3, l=4, m=2, n=3, o=2} reduction={k} need_replication={l, m, o}>
  %0 = "stablehlo.scatter"(%arg0, %arg1, %arg2) ({
    ^bb0(%arg3: tensor<f32>, %arg4: tensor<f32>):
      %1 = stablehlo.add %arg3, %arg4 : tensor<f32>
      stablehlo.return %1 : tensor<f32>
...
func.func @scatter_multiple_input(%arg0: tensor<3x4x2xi32>,
  ...
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([n, k, m], [n, k, m], [i, j, o], [i, j, l, m], [i, j, l, m])->([n, k, m], [n, k, m]) {i=2, j=3, k=4, l=2, m=2, n=3, o=2} reduction={i, j} need_replication={k, l, o}>
...
func.func @scatter_unsupported_multi_reduction_op(%arg0: tensor<3x4x2xi32>,
                                                  ...
                                                  %arg4: tensor<2x3x2x2xf32>)
    -> (tensor<3x4x2xi32>, tensor<3x4x2xf32>) {
  // CHECK: sdy.sharding_rule = #sdy.op_sharding_rule<([n, k, m], [n, k, m], [i, j, o], [i, j, l, m], [i, j, l, m])->([n, k, m], [n, k, m]) {i=2, j=3, k=4, l=2, m=2, n=3, o=2} need_replication={i, j, k, l, o}>
  %0:2 = "stablehlo.scatter"(%arg0, %arg1, %arg2, %arg3, %arg4) ({
    ^bb0(%arg5: tensor<i32>, %arg6: tensor<f32>, %arg7: tensor<i32>, %arg8: tensor<f32>):
      %1 = stablehlo.add %arg5, %arg7 : tensor<i32>
