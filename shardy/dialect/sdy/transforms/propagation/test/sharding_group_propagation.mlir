// RUN: sdy_opt %s -sdy-import-pipeline -sdy-basic-propagate 2>&1 | FileCheck %s
...
func.func @shard_as_applies_despite_barrier(
  ...
  %2 = sdy.propagation_barrier %1 allowed_direction=NONE : tensor<8x8xf32>
  ...
  sdy.sharding_group %3 group_id = 0 : tensor<8x8xf32>
...
func.func @shard_as_across_dataflow_edge(
  ...
  -> tensor<16x16xf32> {
  // CHECK-NEXT: sdy.constant {sdy.sharding = #sdy.sharding_per_value<[<@mesh, [{?}, {"d", ?}]>]>} dense<1.000000e+00> : tensor<16x16xf32>
  ...
  // CHECK: %[[RESULT:.*]]:2 = stablehlo.while(%iterArg = %arg0, %iterArg_0 = %1) : tensor<16x16xf32>, tensor<i32>
