...
// Takes or creates the `TensorShardingPerValueAttr` of the given `op`, but
// replaces the sharding at the given `index` with the given `sharding`.
//
// If no `TensorShardingPerValueAttr` exists, it's replaced with a new one with
// a fully open sharding for each result with the same mesh name as `sharding`.
TensorShardingPerValueAttr replaceShardingPerValue(Operation* op,
                                                   unsigned index,
