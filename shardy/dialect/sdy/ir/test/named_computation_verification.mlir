...
sdy.mesh @meshB = <["a"=2, "b"=2]>

func.func @multiple_meshes(%arg0: tensor<8x2xi32>) -> tensor<8x2xi32> {
  // expected-error@+1 {{op out_shardings shardings can only refer to one mesh: @meshB vs @meshA}}
  %0 = sdy.named_computation<"foo">(%arg0) in_shardings=[<@meshA, [{"b"}, {}]>] out_shardings=[<@meshB, [{"a"}, {}]>] (%arg1: tensor<8x2xi32>) {
    sdy.return %arg1 : tensor<8x2xi32>
  }: (tensor<8x2xi32>) -> tensor<8x2xi32>
  return %0 : tensor<8x2xi32>
}
