...
LogicalResult verifyTensorShardingAttr(TensorShardingAttr shardingAttr,
  ...
  // DO NOT SUBMIT: JAX may output a custom call with a tuple of size 1 during
  // lowering. Which during xla-sdy-import we flatten, propagate, and then in
  // xla-sdy-mhlo-export re-add the tuple since XLA wants it. So we need to
  // allow a single tuple during verification so that the lowered JAX program
  // passes verification.
  if (auto tupleType = dyn_cast<TupleType>(type)) {
    if (tupleType.size() != 1) {
      return emitError("ops can only have a sharding for a tuple of size 1: ") << tupleType;
    }
