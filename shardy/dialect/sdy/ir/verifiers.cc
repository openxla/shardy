...
LogicalResult NamedComputationOp::verify() {
  ...
  SymbolRefAttr meshName;
  if (inShardings && !inShardings->getShardings().empty()) {
    meshName = inShardings->getShardings().front().getMeshSymName();
  } else if (outShardings && !outShardings->getShardings().empty()) {
    meshName = outShardings->getShardings().front().getMeshSymName();
  }
