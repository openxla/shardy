...
class ManualComputationPattern : public OpConversionPattern<CallOp> {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    // bixia todo if (!absl::StrContains(callOp.getCallee(),
    //                       kManualComputationBodyFuncName)) {
    if (callOp.getCallee().find(kManualComputationBodyFuncName) ==
        std::string::npos) {
      return mlir::failure();
    }
...
class SdyRoundTripShardMapImportPass
 ...
 private:
  void runOnOperation() final {
    ...
    // bixia this has the contain change
    target.addDynamicallyLegalOp<CallOp>([](CallOp op) {
      // bixia todo return !absl::StrContains(op.getCallee(), kManualComputationBodyFuncName);
      return op.getCallee().find(kManualComputationBodyFuncName) == std::string::npos;
    });
